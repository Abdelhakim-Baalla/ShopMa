         ...
         @else 
            @foreach($products as $produit)
                <div class="bg-white rounded-lg shadow-lg p-4">
                    ...
                    <p class="text-gray-600 mt-2">{{ Str::limit($produit->description, 60) }}</p>
                    <div class="flex justify-between items-center mt-4">
                        <span class="text-lg font-bold text-gray-900">{{ $produit->prix }} MAD</span>
                        <a href="" class="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-700 transition">Détails</a>
                    </div>
                </div>
        @endforeach
